...
class QuantityInput extends HTMLElement {
  ...
  validateQtyRules() {
    ...
    if (this.input.min) {
      // Alex - updated to target data attr instead of class names
      const buttonMinus = this.querySelector("[data-js-quantity-button][name='minus']");
      buttonMinus.classList.toggle('disabled', parseInt(value) <= parseInt(this.input.min));
    }
    if (this.input.max) {
      const max = parseInt(this.input.max);
      // Alex - updated to target data attr instead of class names
      const buttonPlus = this.querySelector("[data-js-quantity-button][name='plus']");
...
class DrawerDialog extends HTMLElement {
  ...
  show(opener) {
    ...
    if (popup) popup.loadContent();
    trapFocus(this, this.querySelector('[role="dialog"]'));
    // Alex - removing this to prevent StickyHeader error for search
    // window.pauseAllMedia();
  }
  ...
  hide() {
    ...
    // TODO: Alex - this is a fix for PredictiveSearch not triggering "close" because we switched to using DrawerDialog
    // This wouldn't be needed if we weren't setting this.getSearchResults('spf') in PredictiveSearch (to force results automatically on open)
    // Probably better way to do this, but just explaining WHY this is here
    const predictiveSearch = this.querySelector('predictive-search');
    if (predictiveSearch) {
      predictiveSearch.isOpen = false;
    }
  ...
  // Lifting this from MenuDrawer to create a staggered 'menu-closing' class - easier than rewriting everything
  closeAnimation() {
    let animationStart;
    ...
    const handleAnimation = (time) => {
      ...
      if (elapsedTime < 400) {
        ...
        this.removeAttribute('open');
        if (this.closest('details[open]')) {
          trapFocus(this.closest('details[open]'), this.querySelector('summary'));
        }
      }
    };
